...
# Calibrated to: HCUP 2021, NEJM 2017 Sepsis Protocol,
# Surgical SICU Cohort (PMC 2020), WHO Sepsis Factsheet
# ─────────────────────────────────────────────
BASE_P = {
    'S0': {
        'A0': {'S0': 0.95, 'S1': 0.04, 'S2': 0.01, 'S3': 0.00},
        'A1': {'S0': 0.96, 'S1': 0.03, 'S2': 0.01, 'S3': 0.00},
        'A2': {'S0': 0.93, 'S1': 0.04, 'S2': 0.01, 'S3': 0.02},  # iatrogenic risk
        'A3': {'S0': 0.90, 'S1': 0.04, 'S2': 0.02, 'S3': 0.04},  # surgical risk
    },
    'S1': {  # HCUP: 5.6% mortality without tx; ~72% recovery with bundle
        'A0': {'S0': 0.50, 'S1': 0.35, 'S2': 0.09, 'S3': 0.06},
        'A1': {'S0': 0.72, 'S1': 0.21, 'S2': 0.05, 'S3': 0.02},
        'A2': {'S0': 0.67, 'S1': 0.22, 'S2': 0.07, 'S3': 0.04},  # vasopressors not optimal here
        'A3': {'S0': 0.58, 'S1': 0.24, 'S2': 0.09, 'S3': 0.09},  # surgery: high risk if not indicated
    },
    'S2': {  # HCUP: 34.2% mortality; bundle reduces to ~14.9%; SICU: 63% RAP, 33% CCI, 4% early death
        'A0': {'S0': 0.10, 'S1': 0.22, 'S2': 0.35, 'S3': 0.33},
        'A1': {'S0': 0.30, 'S1': 0.28, 'S2': 0.27, 'S3': 0.15},
        'A2': {'S0': 0.45, 'S1': 0.26, 'S2': 0.20, 'S3': 0.09},  # best for shock
        'A3': {'S0': 0.38, 'S1': 0.22, 'S2': 0.22, 'S3': 0.18},  # source-control surgery
    },
    'S3': {  # Absorbing
        'A0': {'S3': 1.0}, 'A1': {'S3': 1.0},
        'A2': {'S3': 1.0}, 'A3': {'S3': 1.0},
...
def apply_risk_modifiers(P_base, R_base, age, is_smoker, has_cvd, has_diabetes, has_immune):
    ...
    # Mortality multipliers (evidence-based)
    ...
    # Recovery modifier (logistic decay)
...
with tab_explain:
    ...
    with col_x2:
        ...
        colors_states = {'S0':'#4ade80','S1':'#fbbf24','S2':'f87171','S3':'#94a3b8'}
        for s in ['S0','S1','S2']:
